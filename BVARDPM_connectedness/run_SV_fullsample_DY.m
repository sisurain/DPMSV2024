...
% with the DPM mechanism stripped out. B0 is a UNIT LOWER-TRIANGULAR
% Cholesky impact matrix (standard Cholesky-SV benchmark, per the run
% summary specification); same per-variable AR(1)
% SV sampler (sample_SV_OISV.m from the Chan-Koop-Yu 2024 OISV codebase);
% same inline GFEVD computation. The differences from the DPM-OISV
% wrapper are the B0 structure (unit lower-triangular Cholesky here vs
% full unrestricted there) and:
%
...
% distribution and the B0 identification scheme (unit lower-triangular
% Cholesky here vs unrestricted order-invariant there); the SV samplers
% and GFEVD computation are identical.
%
...
% 2. Priors for B0 (unit lower-triangular Cholesky)
% Free elements b_ij (i > j) have independent N(0, Vb0) priors; the
% diagonal is fixed at 1 and the upper triangle at 0. With T = 2,673
% daily observations the posterior is data-dominated, so results are
% insensitive to Vb0 within reasonable ranges.
Vb0 = 10;
...
for isim = 1:(nsims + burnin)
    ...
    %----------------------------------------------------------------------
    % Step 3: Sample B0 (unit lower-triangular Cholesky)
    %----------------------------------------------------------------------
    % Row i (i >= 2): u_{i,t} = -sum_{j<i} b_ij u_{j,t} + e_{i,t}, with
    % e_{i,t} ~ N(0, lam_t * exp(h_{i,t})) (lam = 1 in the SV model).
    % Conditional on H, each row is an independent weighted Bayesian
    % regression of u_i on -u_{1:i-1}. Diagonal fixed at 1; upper
    % triangle fixed at 0.
    U_star  = shortY - X*A - mu;
    B0_draw = eye(n);
    for i = 2:n
        w_i = 1 ./ sqrt(lam .* exp(H(:,i)));
        y_i = U_star(:, i)       .* w_i;
        X_i = -U_star(:, 1:i-1)  .* w_i;
        K_bi  = (1/Vb0) * eye(i-1) + X_i' * X_i;
        b_hat = K_bi \ (X_i' * y_i);
        [Cb, flag_chol] = chol(K_bi, 'lower');
        if flag_chol == 0
            % precision K = Cb*Cb'  =>  draw = b_hat + Cb' \ randn
            B0_draw(i, 1:i-1) = (b_hat + Cb' \ randn(i-1, 1))';
        else
            B0_draw(i, 1:i-1) = B0(i, 1:i-1);   % keep previous draw
        end
    end
    B0 = B0_draw;
