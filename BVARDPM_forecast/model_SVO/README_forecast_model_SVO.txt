...
benchmark: the BVAR with stochastic volatility and per-variable
Stock-Watson (2016) outlier states of Carriero, Clark, Marcellino and
Mertens (2024).

Model specification:

  y_t = PAI(L) * y_{t-1} + c + v_t
  v_t = A^{-1} * O_t * Lambda_t^{1/2} * e_t,   e_t ~ N(0, I)
  Lambda_t = diag(exp(h_{1,t}), ..., exp(h_{n,t}))   [random-walk SV]
  O_t = diag(o_{1,t}, ..., o_{n,t})                  [outlier states]
  o_{j,t} in {1, 2, ..., 10} with
    P(o_{j,t} > 1) = pi_j,  P(o_{j,t} = k | o_{j,t} > 1) = 1/9
  pi_j ~ Beta(2.5, 117.5)  (prior outlier frequency of one per
                            four years, per variable)
...
=========================================================================
  ...
  run_SVO_forecast.m
    ...
    Output:  ../results/results_SVO_forecast.mat
...
=========================================================================
  ...
  fcst_SVO_oneorigin.m
    ...
    ordering of the CCMM sampler (mcmcVARSVO.m): VAR coefficients via
    the triangular algorithm; impact matrix A equation by equation;
    and a joint draw of the log volatilities, per-variable outlier
    states, and outlier probabilities via the outlier-augmented KSC
    mixture sampler. After burn-in, each iteration simulates forecast
    paths and evaluates the one-step-ahead joint predictive density.
...
Marcellino and Mertens and are included unmodified.
  ...
  StochVolOutlierKSCcorrsqrt.m
    Joint sampler for correlated random-walk stochastic volatilities,
    Stock-Watson outlier states, and outlier probabilities, using the
    KSC mixture approximation and the precision-based approach.
  ...
  rwnoisePrecisionBasedSampler.m
    ...
    space representation (used by StochVolOutlierKSCcorrsqrt).
...
results_SVO_forecast.mat contains, with N variables, H forecast
