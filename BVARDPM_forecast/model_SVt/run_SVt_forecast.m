...
% run_SVt_forecast.m
%
% Expanding-window forecasting for SV-t (BVAR-t-CSV) from Chan (2020, JBES).
%
% MODEL:
...
%   (longer chain for DPM/Chan models to ensure convergence)
%
% KEY ADVANTAGE: No CTA — uses Normal-IW conjugate sampling.
%   Per-iteration cost is ~20x lower than Cholesky SV models.
%
% REQUIRED FILES:
%   fcst_SVt_oneorigin.m, sample_h.m, sample_nu.m,
%   crpsDraws.m
...
Ndraws             = 1;           % 1 forecast path per MCMC draw
...
%% ---- 6. Main loop ----
fprintf('Starting SV-t forecast: %d origins x %d MCMC iters x %d draws\n', ...
...
parfor ndxT = 1 : Njumpoffs
    ...
    res = fcst_SVt_oneorigin(thisT, Tdata, data, dates_trans, ...
